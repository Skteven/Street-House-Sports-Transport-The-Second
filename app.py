...
def timetable():
    ...
    if session.get('userrole') == 1:
        submissions = conn.execute('SELECT * FROM Timetable JOIN Location ON Timetable.Location = Location.LocationID ORDER BY TimetableID DESC').fetchall()
    else:
        submissions = conn.execute('SELECT * FROM Timetable JOIN Location ON Timetable.Location = Location.LocationID WHERE SubmittedBy = ? ORDER BY TimetableID DESC', (session.get('username'),)).fetchall()
